                ...
                if(FD_ISSET(slaves[i].slave.fromSlavePipe[0],&readSet)){
                    ...
                    if(queue_size == 0){
                        ...
                        if(strcmp(queue[0].buffer,"-2") != 0){
                            ...
                            write(slaves[i].slave.toSlavePipe[1], queue[0].buffer, strlen(queue[0].buffer) + 1);
...
void transformer(const char* command){
    ...
    TransformerInfo slaves[MAX] = calloc(MAX,sizeof(TransformerInfo));
    int maxFd = 0;
    ...
    while(active == 1){
        ...
        /*si me pasen mails para transformar y no tengo en la queue ninguno*/
            ...
            if(queue_size == 0){
                ...
                if(go_ahead == -1 && queue_size < MAX_MAILS){
                queue[queue_size] = queue_message(mail,cID);
                queue_size++;
                }else{
                    slaves[go_ahead].cID = cID;
                    write(slaves[go_ahead].slave.toSlavePipe[1], mail, strlen(mail) + 1);
                }
                
                /* Si no hay slaves libres, aun no tengo el maximo adimitido, creo uno y lo  pongo a trabajar*/
                if(active_slaves < MAX && queue_size > 0){
                    slaves[active_slaves].slave = transform(mail,command);
                    
                    close(slaves[active_slaves].slave.fromSlavePipe[1]);
                    close(slaves[active_slaves].slave.toSlavePipe[0]);

                    FD_SET(slaves[active_slaves].slave.fromSlavePipe[0], &readFromSlaves);
                    if (slaves[active_slaves].slave.fromSlavePipe[0] > maxFd) {
                        maxFd = slaves[active_slaves].slave.fromSlavePipe[0];
                    }
                    slaves[active_slaves].cID = cID;
