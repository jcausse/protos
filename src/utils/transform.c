...
static int transform_mail(char * mailDir, char * receiverMail, char * senderMail, char * command, char * toSave) {
    int toSaveFd = open(toSave, O_CREAT | O_EXCL | O_WRONLY , MODE_T);

    if(toSaveFd == ERR) return ERR;
    ...
    for(int i = 0; i < BUFF_SIZE && buff[i] != '\0'; i++) buff[i] = '\0';
    snprintf(buff, 320, "%s %s > %s 2> transform.err", command, mailDir, toSave);

    int retVal = system(buff);

    toSaveFd = open(toSave, O_CREAT | O_EXCL | O_WRONLY , MODE_T);
    if(toSaveFd == ERR) return ERR;
...
int transform(bool enabled, char * cmd, char * mailDir, char * receiverMail, char * senderMail, char * fileName){
    ...
    int transform;
    if(enabled){
        transform = transform_mail(mailDir, receiverMail, senderMail, cmd, toSave);
    }
    else {
        transform = send_mail(mailDir, receiverMail, senderMail, toSave);
    }
